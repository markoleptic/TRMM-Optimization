...
// Assuming A_ip and B_pj are 4-element vectors (float) for SSE
__m128 trmm_sse(const float *A_ip, const float *B_pj)
{
    __m128 result = _mm_setzero_ps(); // Initialize result to zero

    for (int i = 0; i < 4; ++i)
    {
        __m128 a = _mm_load_ps(&A_ip[i * 4]);          // Load 4 elements from A_ip
        __m128 b = _mm_set1_ps(B_pj[i]);               // Broadcast B_pj[i] to a vector
        result = _mm_add_ps(result, _mm_mul_ps(a, b)); // Multiply and accumulate
    }
...

{
    ...
    */
    ...
    if (rid == root_rid)
    {
        ...
        {
            for (int p0 = 0; p0 < m0; p0 += 8)
            {
                __m256 b = _mm256_loadu_ps(&B_distributed[p0 * cs_B + j0 * rs_B]);
                for (int i0 = 0; i0 < j0; i0 += 8)
                {
                    __m256 a = _mm256_loadu_ps(&A_distributed[i0 * cs_A + p0 * rs_A]);
                    __m256 c = _mm256_add_ps(_mm256_loadu_ps(&C_distributed[i0 * cs_C + j0 * rs_C]), _mm256_mul_ps(a, b));
                    //__m256 c = _mm256_fmadd_ps(_mm256_loadu_ps(&C_distributed[i0 * cs_C + j0 * rs_C]), a, b);
                    _mm256_storeu_ps(&C_distributed[i0 * cs_C + j0 * rs_C], c);
                }
            }
        }
    }
